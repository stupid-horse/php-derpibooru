...
//NOTES
/* Check the database for duplicate entries: select distinct id_number, count(id) from images group by id_number having count(id) > 1


DELETE n1 FROM images n1, images n2 WHERE n1.uniqid > n2.uniqid AND n1.id_number = n2.id_number


select distinct id_number, count(id) from images group by id_number having count(id) > 1
*/
...
//	$sqlCheckForDuplicate = "SELECT `uniqid` FROM `images` WHERE `expected_id_number` = '".$expected_id_number."';";
//	if(mysql_num_rows(mysql_query($sqlCheckForDuplicate)) == 0 ){
	if(!in_array($expected_id_number, $indb)){
		...
		} else 
		{
			...
			// Naming Scheme $array['posts'][$POSTNUMBER][requestedelement]
